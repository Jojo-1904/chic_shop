...
class AdminHomeController extends AbstractController
{
    ...
    {
        // On instancier un nouvel objet Home
        $home = new Home();
        // On crée un formulaire de home en utilisant la classe HomeType
        $form = $this->createForm(HomeType::class, $home);
        // On traite le formulaire (hydratation)
        $form->handleRequest($request);// hydratation de l'objet $home ave des les données se trouvant dans le$request
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            $entityManager->flush();
            // On crée un message flash (le message flash est mis en session et est supprimé dés qu'il est affiché)
            $this->addFlash('success', 'La home a bien été créé');
            // On redirige vers la list des homes
